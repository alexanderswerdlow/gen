...
def visualize_input_data(batch: InputData, name: Optional[str] = None, names: Optional[list[str]] = None, show_background_foreground_only: bool = False, show_overlapping_masks: bool = False):
    ...
    gen_one_hot = integer_to_one_hot(batch.gen_segmentation, add_background_channel=False)
    disc_one_hot = integer_to_one_hot(batch.disc_segmentation, add_background_channel=False)
    ...
    for b in range(batch.bs):
        ...
        gen_ = Im.concat_vertical(
            ...
            Im(onehot_to_color(gen_one_hot[b].squeeze(0), override_colors=override_colors)),
        )
        if batch.gen_grid is not None:
            gen_ = Im.concat_vertical(
        ...
        disc_ = Im.concat_vertical(
            ...
            Im(onehot_to_color(disc_one_hot[b].squeeze(0), override_colors=override_colors)),
        )
        ...
        if show_overlapping_masks:
            masks = gen_one_hot.permute(0, 3, 1, 2)[b]
            ...
            first_masks = Im(masks.unsqueeze(-1)).grid(pad_value=0.5)
