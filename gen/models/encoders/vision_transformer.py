...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.final_blocks = nn.Sequential(*[
            ...
            for i in range(2)])
        self.final_norm = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
    ...
    def forward_features_custom(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        ...
        with torch.no_grad():
            ...
            x = self.norm(x)
            final_output = x.detach().clone()

        final_output = torch.cat((y, final_output), dim=1)
        tmp_mid = torch.cat((y, tmp_mid), dim=1)

        if self.grad_checkpointing and not torch.jit.is_scripting():
            tmp_mid = checkpoint_seq(self.mid_blocks, tmp_mid)
        else:
            tmp_mid = self.mid_blocks(tmp_mid)

        if self.grad_checkpointing and not torch.jit.is_scripting():
            final_output = checkpoint_seq(self.final_blocks, final_output)
        else:
            final_output = self.final_blocks(final_output)

        final_output = self.final_norm(final_output)

        return x, tmp_mid, final_output
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        return self.forward_features_custom(x, y)
