...
class BaseMapper(nn.Module):
    ...
    def prepare_for_training(self, weight_dtype: torch.dtype, accelerator: Accelerator, bypass_dtype_check: bool = False):
        ...
        if self.cfg.model.freeze_neti_mapper:
    ...
    def get_hidden_state(self, batch, timesteps, dtype, device, per_timestep: bool = False, disable_conditioning: bool = False):
        ...
        if not disable_conditioning:
            ...
            for i in range(bs):
                if self.cfg.model.encode_token_without_tl:
                    original = batch["gen_segmentation"][i].new_ones(
                        (1, batch["gen_segmentation"][i].shape[0], batch["gen_segmentation"][i].shape[1])
                    )
                    ...
                    if original.shape[0] == 0:
                        original = (
                            batch["gen_segmentation"][i]
                            .new_ones((1, batch["gen_segmentation"][i].shape[0], batch["gen_segmentation"][i].shape[1]))
                            .cpu()
                        )
                    else:
        ...
        if self.cfg.model.per_timestep_conditioning:
            if per_timestep:
                ...
                _hs = self.get_neti_text_embedding_for_batch(input_ids=batch["input_ids"], timesteps=timesteps, device=device, **text_encoder_dict)
        else:
            assert self.cfg.model.output_bypass is False
            neti_batch = NeTIBatch(
                input_ids=batch["input_ids"],
                placeholder_token_id=self.placeholder_token_id,
                timesteps=timesteps,
                unet_layers=torch.tensor(0, device=device).repeat(timesteps.shape[0]),
            )
            layer_hidden_state, layer_hidden_state_bypass = self.text_encoder(batch=neti_batch, **text_encoder_dict)
            _hs = layer_hidden_state[0].to(dtype=self.weight_dtype)

        return _hs, input_prompt

    def forward(self, batch, noisy_latents, timesteps, weight_dtype):
        encoder_hidden_states, input_prompt = self.get_hidden_state(
