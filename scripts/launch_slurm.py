...
class Task:
    ...
    def __call__(self):
        ...
        with open(Path(self.cfg.output) / 'env_vars.txt', 'w') as file:
        ...
        num_processes = self.cfg.slurm.n_processes * self.cfg.slurm.n_nodes # --num_processes {num_processes}
        machine_rank = dist_env.rank // self.cfg.slurm.n_processes
        deepspeed_str = ""
        if self.cfg.slurm.use_deepspeed:
            deepspeed_str = f" {DEEPSPEED_MULTINODE} --use_deepspeed"

        if self.cfg.use_accelerate:
            init_cmd = f"accelerate launch --mixed_precision bf16 --dynamo_backend no --gpu_ids all {deepspeed_str} --num_machines {self.cfg.slurm.n_nodes} --machine_rank {machine_rank} --main_process_ip {dist_env.master_addr} --main_process_port {dist_env.master_port}"
        else:
            init_cmd = f"python"

        cmd = f"{init_cmd} {self.cfg.program} {self.cfg.slurm.cmd}"
...
@hydra.main(version_base=None, config_path="diff_tta/config", config_name="slurm_config")
def main(cfg: DictConfig) -> None:
    with open_dict(cfg):
        cfg.output = hydra.core.hydra_config.HydraConfig.get().runtime.output_dir
    executor = submitit.AutoExecutor(folder=cfg.output, max_num_timeout=cfg.slurm.max_num_timeout)
    print_config(cfg)
