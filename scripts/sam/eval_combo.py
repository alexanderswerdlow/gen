...
import torch
import fire
from experiments.save_custom_json import save_json, update_with_inference_batch
from scripts.sam.data import build_data, setup_coco_img_ids
import math
    ...
    with torch.autograd.profiler.record_function("timed region"):
        ...
        with torch.autograd.profiler.record_function("nt predict_torch"):
            ...
            if annotations is not None:
                # breakpoint()
                inference_batch = [create_result_entry_inference(d[0], g, m, s, d[3]) for (m, s, d, g) in zip(masks.unbind(),
...
def build_results(batched_data_iter,
    ...
    for batch in tqdm.tqdm(batched_data_iter):
        with torch.no_grad():
            ...
            result_batch, num_datapoints, kernel_time, annotations, annotation_id = batch_runner(predictor, batch, batch_size, pad_input_image_batch, annotations=annotations, annotation_id=annotation_id)
    ...
    return results, avg_ms_per_img, num_batches, num_images, annotations
...
def run(
    ...
    batched_data_iter = torch.utils.data.DataLoader(idx,
    ...
    results, avg_ms_per_img, num_batches, num_images, annotations = runner(build_results,
                                                                            batched_data_iter,
                                                                            predictor,
                                                                            mask_debug_out_dir,
                                                                            batch_size,
